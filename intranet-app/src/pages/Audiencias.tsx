  ...
  return (
    <div className="max-w-6xl space-y-6">
      <div>
        ...
        <p className="text-text-secondary text-sm mt-1">Acompanhe as audiências agendadas do escritório.</p>
      </div>
      ...
      <Card padded={false} className="p-4 flex flex-col sm:flex-row gap-3">
        <input
          ...
          className="flex-1 bg-cream border border-border rounded-lg px-3.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gold/40"
        />
        <select
          value={periodo}
          onChange={(e) => setPeriodo(e.target.value as FiltroPeriodo)}
          className="bg-cream border border-border rounded-lg px-3.5 py-2 text-sm text-navy focus:outline-none focus:ring-2 focus:ring-gold/40"
        >
        ...
        {filtradas.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtradas.map((h) => (
                  <tr key={h.id} className="border-b border-border last:border-0 hover:bg-cream/60">
