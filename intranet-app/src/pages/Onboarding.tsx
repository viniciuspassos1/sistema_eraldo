        ...
        <ul className="space-y-2">
          {checklist.map((c, i) => (
            <li key={c.item}>
              <button
                onClick={() =>
                  setChecklist((prev) => prev.map((p, idx) => (idx === i ? { ...p, feito: !p.feito } : p)))
                }
                className="w-full flex items-center gap-3 text-left px-3 py-2.5 rounded-lg hover:bg-cream"
              >
                <span
                ...
                <span className={`text-sm ${c.feito ? 'text-text-secondary line-through' : 'text-navy'}`}>
                  {c.item}
                </span>
              </button>
            </li>
